from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..models.bind_sync_settings_synconchanges import BINDSyncSettingsSynconchanges
from ..models.bind_sync_settings_synctimeout import BINDSyncSettingsSynctimeout
from ..types import UNSET, Unset

T = TypeVar("T", bound="BINDSyncSettings")


@_attrs_define
class BINDSyncSettings:
    """
    Attributes:
        synconchanges (BINDSyncSettingsSynconchanges | Unset): The sync mode to use.<br>
        synctimeout (BINDSyncSettingsSynctimeout | Unset): The timeout for the sync process.<br> Default:
            BINDSyncSettingsSynctimeout.VALUE_30.
        masterip (str | Unset): The IP address of the master BIND server.<br>
    """

    synconchanges: BINDSyncSettingsSynconchanges | Unset = UNSET
    synctimeout: BINDSyncSettingsSynctimeout | Unset = BINDSyncSettingsSynctimeout.VALUE_30
    masterip: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        synconchanges: str | Unset = UNSET
        if not isinstance(self.synconchanges, Unset):
            synconchanges = self.synconchanges.value

        synctimeout: int | Unset = UNSET
        if not isinstance(self.synctimeout, Unset):
            synctimeout = self.synctimeout.value

        masterip = self.masterip

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if synconchanges is not UNSET:
            field_dict["synconchanges"] = synconchanges
        if synctimeout is not UNSET:
            field_dict["synctimeout"] = synctimeout
        if masterip is not UNSET:
            field_dict["masterip"] = masterip

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _synconchanges = d.pop("synconchanges", UNSET)
        synconchanges: BINDSyncSettingsSynconchanges | Unset
        if isinstance(_synconchanges, Unset):
            synconchanges = UNSET
        else:
            synconchanges = BINDSyncSettingsSynconchanges(_synconchanges)

        _synctimeout = d.pop("synctimeout", UNSET)
        synctimeout: BINDSyncSettingsSynctimeout | Unset
        if isinstance(_synctimeout, Unset):
            synctimeout = UNSET
        else:
            synctimeout = BINDSyncSettingsSynctimeout(_synctimeout)

        masterip = d.pop("masterip", UNSET)

        bind_sync_settings = cls(
            synconchanges=synconchanges,
            synctimeout=synctimeout,
            masterip=masterip,
        )

        bind_sync_settings.additional_properties = d
        return bind_sync_settings

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
