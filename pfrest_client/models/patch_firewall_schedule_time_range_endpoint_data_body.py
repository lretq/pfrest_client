...
T = TypeVar("T", bound="PatchFirewallScheduleTimeRangeEndpointDataBody")


@_attrs_define
class PatchFirewallScheduleTimeRangeEndpointDataBody:
    """
    Attributes:
        ...
        month (list[FirewallScheduleTimeRangeMonthItem] | Unset): The month for each specified `day` value. Each value
            specified must correspond with a `day` field value and must match the order exactly. For example, a `month`
            value of `[3, 6]` and a `day` value of `[2, 17]` would evaluate to March 2nd and June 17th
            respectively.<br><br>This field is only available when the following conditions are met:<br>- `position` must be
            equal to `NULL`<br>
        day (list[int] | Unset): The day for each specified `month` value. Each value specified must correspond with a
            `month` field value and must match the order exactly. For example, a `month` value of `[3, 6]` and a `day` value
            of `[2, 17]` would evaluate to March 2nd and June 17th respectively.<br><br>This field is only available when
            the following conditions are met:<br>- `position` must be equal to `NULL`<br>
        hour (str | Unset): The start time and end time for this time range in 24-hour format (i.e. HH:MM-HH:MM).<br>
    ...
    month: list[FirewallScheduleTimeRangeMonthItem] | Unset = UNSET
    day: list[int] | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        day = cast(list[int], d.pop("day", UNSET))

        hour = d.pop("hour", UNSET)

        rangedescr = d.pop("rangedescr", UNSET)

        patch_firewall_schedule_time_range_endpoint_data_body = cls(
            parent_id=parent_id,
            id=id,
            position=position,
            month=month,
        ...
        patch_firewall_schedule_time_range_endpoint_data_body.additional_properties = d
        return patch_firewall_schedule_time_range_endpoint_data_body
