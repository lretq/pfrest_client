...
T = TypeVar("T", bound="PatchFirewallNATPortForwardEndpointDataBody")


@_attrs_define
class PatchFirewallNATPortForwardEndpointDataBody:
    """
    Attributes:
        id (int): The ID of the object or resource to interact with.
        interface (str | Unset): The interface this port forward rule applies to.<br>
        ...
        protocol (PortForwardProtocol | Unset): The IP/transport protocol this port forward rule should match.<br>
        source (str | Unset): The source address this port forward rule applies to. Valid value options are: an existing
        ...
        destination (str | Unset): The destination address this rule applies to. Valid value options are: an existing
            interface, an IP address, a subnet CIDR, an existing alias, `any`, `(self)`, `l2tp`, `pppoe`. The context of
            this address can be inverted by prefixing the value with `!`. For interface values, the `:ip`  modifier can be
            appended to the value to use the interface's IP address instead of its entire subnet.<br>
        ...
        target (str | Unset): The IP address or alias of the internal host to forward matching traffic to. Valid value
            options are: an IP address, an existing alias. For interface values, the `:ip`  modifier can be appended to the
            value to use the interface's IP address instead of its entire subnet.<br>
        local_port (str | Unset): The port on the internal host to forward matching traffic to. In most cases, this must
            match the `destination_port` value. In the event that the `desintation_port` is a range, this value should be
            the first value in that range. Valid options are: a TCP/UDP port number, an existing port type firewall
    ...
    protocol: PortForwardProtocol | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        protocol: PortForwardProtocol | Unset
        if isinstance(_protocol, Unset):
            protocol = UNSET
        else:
            protocol = PortForwardProtocol(_protocol)
        ...
        patch_firewall_nat_port_forward_endpoint_data_body = cls(
            id=id,
            interface=interface,
        ...
        patch_firewall_nat_port_forward_endpoint_data_body.additional_properties = d
        return patch_firewall_nat_port_forward_endpoint_data_body
