...
T = TypeVar("T", bound="PostVPNOpenVPNServerEndpointJsonBody")


@_attrs_define
class PostVPNOpenVPNServerEndpointJsonBody:
    """
    Attributes:
        mode (OpenVPNServerMode): The OpenVPN server mode.<br>
        dev_mode (OpenVPNServerDevMode): The carrier mode for this OpenVPN server. `tun` mode carries IPv4 and IPv6
            (layer 3) and is the most common and compatible mode across all platforms. `tap` mode is capable of carrying
            802.3 (layer 2).<br>
        protocol (OpenVPNServerProtocol): The protocol used by this OpenVPN server.<br>
        interface (str): The interface or Virtual IP address where OpenVPN will receive client connections.<br><br>This
            field is only available when the following conditions are met:<br>- `protocol` must not be one of [ UDP, TCP
            ]<br>
        tls_type (OpenVPNServerTlsType): The TLS key usage type. In `auth` mode, the TLS key is used only as HMAC
        ...
        certref (str): The `refid` of the certificate object to assume as the OpenVPN server certificate.<br>
        dh_length (str): The Diffie-Hellman (DH) parameter set used for key exchange.<br>
        ecdh_curve (str): The Elliptic Curve to use for key exchange. The curve from the server certificate is used by
            default when the server uses an ECDSA certificate. Otherwise, secp384r1 is used as a fallback.<br>
        data_ciphers (list[str]): The encryption algorithms/ciphers allowed by this OpenVPN server.<br>
        ...
        digest (str): The algorithm used to authenticate data channel packets, and control channel packets if a TLS Key
            is present.<br>
        serverbridge_interface (str): The interface to which this TAP instance will be bridged. This is not done
        ...
        serverbridge_dhcp_start (str): The bridge DHCP range's start address.<br><br>This field is only available when
            the following conditions are met:<br>- `serverbridge_dhcp` must be equal to `true`<br>
        serverbridge_dhcp_end (str): The bridge DHCP range's end address.<br><br>This field is only available when the
            following conditions are met:<br>- `serverbridge_dhcp` must be equal to `true`<br>
    ...
    mode: OpenVPNServerMode
    dev_mode: OpenVPNServerDevMode
    protocol: OpenVPNServerProtocol
    interface: str
    tls_type: OpenVPNServerTlsType
    caref: str
    certref: str
    dh_length: str
    ecdh_curve: str
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        mode = OpenVPNServerMode(d.pop("mode"))

        dev_mode = OpenVPNServerDevMode(d.pop("dev_mode"))

        protocol = OpenVPNServerProtocol(d.pop("protocol"))

        interface = d.pop("interface")

        tls_type = OpenVPNServerTlsType(d.pop("tls_type"))
        ...
        post_vpn_open_vpn_server_endpoint_json_body = cls(
            mode=mode,
            dev_mode=dev_mode,
            protocol=protocol,
            interface=interface,
        ...
        post_vpn_open_vpn_server_endpoint_json_body.additional_properties = d
        return post_vpn_open_vpn_server_endpoint_json_body
